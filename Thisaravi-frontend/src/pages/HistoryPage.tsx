...
import { fetchMyHistory } from '@/services/feedbackService';
import { parseResponse } from '@/lib/parsers';
import { useAuth } from '@/context/AuthContext';
import type { HistoryEntry, FeedbackEntry } from '@/types/api';
...
function HistoryCard({ entry, index }: { entry: HistoryEntry; index: number }) {
  ...
  // Try to get match % for the card preview
      ...
      {!isLoading && !error && history.length === 0 && (
        <Card>
          <CardContent className="py-14 flex flex-col items-center gap-3 text-center text-muted-foreground">
            ...
            <p className="text-sm">
              Run your first skill gap analysis from the <strong>Analysis</strong> page and it will
              appear here.
            </p>
          </CardContent>
        </Card>
